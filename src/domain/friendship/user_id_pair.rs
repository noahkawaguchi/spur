...
impl UserIdPair {
    ...
    pub fn is_lesser(self, id: i32) -> Result<bool> {
        match (id == self.0, id == self.1) {
            (true, true) => {
                Err(anyhow!("Internal logic error: UserIdPair distinct invariant broken"))
            }
            (false, false) => Err(anyhow!(
                "Internal logic error: UserIdPair `is_lesser` was erroneously passed an \
                 irrelevant ID"
            )),
            (true, false) => Ok(true),
            (false, true) => Ok(false),
...
mod tests {
    ...
    fn rejects_equal_ids() {
        ...
        assert_matches!(result, Err(FriendshipError::SelfFriendship));
    }
