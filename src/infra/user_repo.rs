...
mod tests {
    ...
    #[test]
    fn sets_auto_generated_id_and_created_at() {
        tokio_test(async {
            with_test_pool(|pool| async move {
                let repo = PgUserRepo;
                ...
                for (i, user) in make_test_users().into_iter().enumerate() {
                    ...
                    let expected_id: i32 =
                        (i + 1).try_into().expect("failed to cast usize into i32");
    ...
    #[test]
    fn rejects_empty_and_blank_fields() {
        tokio_test(async {
            with_test_pool(|pool| async move {
                let repo = PgUserRepo;
    ...
    #[test]
    fn rejects_usernames_with_illegal_characters() {
        tokio_test(async {
            with_test_pool(|pool| async move {
                let repo = PgUserRepo;
                let bad_usernames = [
