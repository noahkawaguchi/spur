...
pub fn anchor_offset(days: u32, hours: u32, minutes: u32) -> Result<DateTime<Utc>> {
    ...
    let offset = TimeDelta::try_days(days.into())
        .and_then(|d| d.checked_add(&TimeDelta::try_hours(hours.into())?))
        .and_then(|dh| dh.checked_add(&TimeDelta::try_minutes(minutes.into())?))
        .context("Failed to create offset TimeDelta")?;

    reiwa
        .checked_add_signed(offset)
        .with_context(|| format!("Overflowed adding offset to anchor date: {reiwa} + {offset}"))
}
